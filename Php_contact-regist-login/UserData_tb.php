...
if (!empty($_SESSION['id'])) {
    ...
    //$stmt = "SELECT * FROM registerlogin";
    //$query = $conn->query($stmt);
    if (isset($_GET['submit'])) {
        $filter = $_GET['filter'];
        $stmt = "SELECT * FROM registerlogin WHERE concat(user_id, user_name, user_email, user_password, reg_date) LIKE '%$filter%'";
        $query= $conn->query($stmt);
    } else {
        $stmt = "SELECT * FROM registerlogin";
        $query = $conn->query($stmt);
